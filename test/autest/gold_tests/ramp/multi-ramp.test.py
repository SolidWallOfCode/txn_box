...
Test.Summary = '''
Multi path differential ramping / bucketing.
'''

class State:
    RepeatCount = 1000
    TxnCount = 6; # transactions per repeat.
    ...
    def validate(self, log_path):
        ...
        for r in self.ramps.items():
            ...
            if target == 0:
                ...
                lower = upper = self.RepeatCount
            else:
                lower = int((self.RepeatCount * (r[1][0]-2)) / 100)
                upper = int((self.RepeatCount * (r[1][0]+2)) / 100)
