...
bool Mod_query_sort::is_valid_for(const ActiveType &ex_type) const
{
  return ex_type.can_satisfy(STRING);
}

ActiveType Mod_query_sort::result_type(const ActiveType &) const
{
  return { NIL, STRING };
}

Rv<Feature> Mod_query_sort::operator()(Context &ctx, feature_type_for<STRING> qs)
{
...
public:
  ...
  class Ex_Name : public Extractor {
    virtual swoc::Rv<ActiveType> validate(Config &cfg, Spec &spec, swoc::TextView const &);
...
bool Mod_query_filter::is_valid_for(const ActiveType &ex_type) const
{
  return ex_type.can_satisfy(STRING);
}

ActiveType Mod_query_filter::result_type(const ActiveType &) const
{
  return { NIL, STRING };
}
...
Mod_query_filter::compare(Context &ctx, QPair const& qp) const -> Case const *
{
  ...
  for (auto const &c : _cases) {
    if ((!c._cmp || (*c._cmp)(ctx, fstr(qp.name))) &&
        (!c._value_cmp || (*c._value_cmp)(ctx, fstr(qp.value)))) {
      return &c;
    }
  }
  return nullptr;
...
Rv<Modifier::Handle> Mod_query_filter::load(Config &cfg, YAML::Node node, TextView, TextView, YAML::Node key_value)
{
...
Rv<Feature> Mod_query_filter::operator()(Context &ctx, feature_type_for<STRING> qs) {
  using list_type = swoc::IntrusiveDList<QPair::Linkage>;
  auto list = ctx.make<list_type>();
